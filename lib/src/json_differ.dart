...
class JsonDiffer {
  ...
  JsonDiffer(String leftString, String rightString) {
    Object _leftJson = JsonDecoder().convert(leftString);
    Object _rightJson = JsonDecoder().convert(rightString);
  ...
  bool _deepEquals(e1, e2) => DeepCollectionEquality().equals(e1, e2);
      ...
      if (!_deepEquals(left[leftHand], right[rightHand])) {
        ...
        if (!foundMissing) {
          ...
          if (parentKey != null &&
            ...
            node[leftFoot.toString()] = _diffObjects(leftObject, rightObject);
          } else if (leftObject is List && rightObject is List) {
            node[leftFoot.toString()] =
                _diffLists(leftObject, rightObject, null);
          } else {
            node.changed[leftFoot.toString()] = [leftObject, rightObject];
          }
        }
